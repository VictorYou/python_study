...
def run_lab_reservation_from_container(container_name, query, url ,nok_case="no_nok_case"):
    """
    runs lab request command with given container name, query and url. Raises exception
    if reservation fails
    :return: Labname
    """
    args = ['docker', 'exec', container_name, 'python', '/src/client.py', '-l', 'INFO', '--username',
            "test-user", '-url', 'http://web:8000', 'request-lab', "-wait", "5", '--message', '"test"', '-q',
            query]
    if url != "":
        args[10] = url
...
def release_lab(lab_name, container_name, url="", nok_case="no_nok_case"):
    """
    ...
    args = ['docker', 'exec', container_name, 'python', '/src/client.py', '-l', 'INFO',
            '--username', 'test-user', '-url', 'http://web:8000', 'release-lab', '--lab-name', lab_name]
    if url != "":
        args[10] = url
    ...
    try:
        ...
        raise Exception("ERROR: Invalid container name '" + container_name + "'. Exit code: '1'")


def get_container_name():
    """
    Runs command that resolves name of container.

    :return: container name
    """

    args = "docker ps | grep stateservice_labrequest_worker | awk '{print $1}'"
    p = subprocess.Popen(args, stdout=subprocess.PIPE, shell=True, universal_newlines=True)
    container = ""
...
def create_test_data(container_name):
    """
    Runs create test labs command. Used for test suite setup
    :param container_name: 
    """
    _command_executer(
        ['docker', 'exec', container_name, 'python', '/src/client.py', '-l', 'INFO',
         '--username', 'test-user', '-url', 'http://web:8000', 'create-test-labs']
    )


def find_lab_query(container_name):
    """
    ...
    queries = []
    args = ['docker', 'exec', container_name, 'python', '/src/client.py', '-l', 'INFO',
            '--username', 'test-user', '-url', 'http://web:8000', 'show-labs']
...
def preserve_lab_state(container, lab_name):
    """
    Runs preserve-state command and adds new information for the new state
    :return: 
    """
    args = [
        'docker', 'exec', container, 'python', '/src/client.py', '-l',
        'INFO', '-u', 'test-user', '-url', 'http://web:8000', 'preserve-state',
...
def find_lab_name(container_name):
    """
...
def release_lab_with_wrong_username(lab_name, container_name, url=""):
    """
    ...
    args = ['docker', 'exec', container_name, 'python', '/src/client.py', '-l', 'INFO',
            '--username', 'wrong-user-name', '-url', 'http://web:8000', 'release-lab', '--lab-name', lab_name]
    if url != "":
        args[11] = url
...
def user_reserves_lab_py(container_name, query, url):
    """
    Python keyword for lab reservation E2E test

    """
    lab_name = run_lab_reservation_from_container(container_name, query, url)
    if lab_name == "<none>":
...
def user_reserves_lab_whit_wrong_query_and_fails_py(container_name, url):
    """
    ...
    try:
        message = run_lab_reservation_from_container(container_name, "Wrong_Query", url, nok_case="'Wrong_Query'")
        print(message)
...
def user_reserves_reserved_lab_and_fails_py(container_name, web, ):
    """
    Python keyword for reserve already reserved lab NOK case

    """
    query = find_lab_query(get_container_name())
    run_lab_reservation_from_container(container_name, query, web)
    try:
        message = run_lab_reservation_from_container(container_name, query, web, nok_case="'no_lab_available'")
        if len(message.split(" ")) == 1:
...
def user_releases_lab_reserved_for_another_user_and_fails(container, web):
    """
    Python keyword for release lab reserved for another user NOK case

    """
    query = find_lab_query(container)
    lab_name = run_lab_reservation_from_container(container, query, web)
    try:
        release_lab_with_wrong_username(lab_name, container, web)
        raise Exception("ERROR: Lab release succeed with wrong username. Exit code: '1'")
    except NOKError:
        return


def user_releases_unreserved_lab_and_fails_py(container, url):
    """
    Python keyword for release unreserved lab NOK case

    """
    lab_name = find_lab_name(container)
    print(lab_name)
    try:
        release_lab(lab_name, container, url, nok_case="release unreserved")
        raise Exception("ERROR: Lab release succeed to unreserved lab. Exit code: '1'")
    except NOKError:
        return


def user_preserves_state_of_unreserved_lab_and_fails_py(container):
    """
    ...
    lab_name = find_lab_name(container)
    message = preserve_lab_state(container, lab_name)
...
def admin_creates_lab_py(container, labname="sprintlabtest1232"):
    commands = ["docker", "exec", container, "python", "/src/client.py", "-u", "admin", "-url", "http://web:8000", "admin-add-lab", "-lab", labname]
...
def admin_adds_lab_that_already_exists_and_fails_py(container):
    existing_lab = find_lab_name(container)
    print("found existing labb '" + existing_lab + "'")
    output, exitcode = admin_creates_lab_py(container, existing_lab)
...
def admin_removes_lab_py(container, lab_name="sprintlabtest1232"):
    commands = ["docker", "exec", container, "python", "/src/client.py", "-u", "admin", "-url", "http://web:8000", "admin-remove-labs", "-lab", lab_name, "-f"]
    _command_executer(commands)
...
def user_removes_lab_and_fails_py(container):
    lab_name = find_lab_name(container)
    commands = ["docker", "exec", container, "python", "/src/client.py", "-u", "testuser", "-url", "http://web:8000", "admin-remove-labs", "-lab", lab_name, "-f"]
